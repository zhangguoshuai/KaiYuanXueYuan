...
public class AlbumAndArtisDetailsActivity extends AppCompatActivity implements View.OnClickListener, ObservableScrollViewCallbacks,
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.bottombar_play){
            ...
            MusicPreferance.setShuffel(getApplicationContext(), (v.isSelected() ? true : false));
            MediaController.getInstance().shuffleList(MusicPreferance.playlist);
            DMPlayerUtility.changeColorSet(getApplication(), (ImageView) v, v.isSelected());
        }
        else if(id == R.id.btn_toggle){
            v.setSelected(v.isSelected() ? false : true);
            MediaController.getInstance().repeatMode = v.isSelected() ? 1 : 0;
            MusicPreferance.setRepeat(getApplication(), (v.isSelected() ? 1 : 0));
            DMPlayerUtility.changeColorSet(getApplicationContext(), (ImageView) v, v.isSelected());
        }
        else if(id == R.id.bottombar_img_Favorite){
            if (MediaController.getInstance().getPlayingSongDetail() != null) {
                MediaController.getInstance().storeFavoritePlay(getApplicationContext(), MediaController.getInstance().getPlayingSongDetail(), v.isSelected() ? 0 : 1);
                v.setSelected(v.isSelected() ? false : true);
    ...
    public void theme() {
        ...
        DMPlayerUtility.settingTheme(getApplicationContext(), theme);
    }
    ...
    private void initialize() {
        ...
        mAllSongsListAdapter = new AllSongsListAdapter(getApplicationContext());
        recycler_songslist.setAdapter(mAllSongsListAdapter);

        options = new DisplayImageOptions.Builder().showImageOnLoading(R.mipmap.bg_default_album_art)
    ...
    private void loadAlbumSongs(long id) {
        PhoneMediaControl mPhoneMediaControl = new PhoneMediaControl();
        ...
        mPhoneMediaControl.loadMusicList(getApplicationContext(), id, PhoneMediaControl.SonLoadFor.Album, "");
    ...
    private void loadArtisSongs(long id) {
        ...
        mPhoneMediaControl.loadMusicList(getApplicationContext(), id, PhoneMediaControl.SonLoadFor.Artis, "");
    }
    ...
    private void loadGenersSongs(long id) {
        ...
        mPhoneMediaControl.loadMusicList(getApplicationContext(), id, PhoneMediaControl.SonLoadFor.Gener, "");
    }
    ...
    private void initiSlidingUpPanel() {
        ...
        imgbtn_toggle.setSelected((MusicPreferance.getRepeat(getApplicationContext()) == 1) ? true : false);
        MediaController.getInstance().shuffleMusic = imgbtn_toggle.isSelected() ? true : false;
        DMPlayerUtility.changeColorSet(getApplicationContext(), imgbtn_toggle, imgbtn_toggle.isSelected());

        imgbtn_suffel.setSelected(MusicPreferance.getShuffel(getApplicationContext()) ? true : false);
        MediaController.getInstance().repeatMode = imgbtn_suffel.isSelected() ? 1 : 0;
        DMPlayerUtility.changeColorSet(getApplicationContext(), imgbtn_suffel, imgbtn_suffel.isSelected());
    ...
    private void loadAlreadyPaing() {
        ...
        if (mSongDetail != null) {
            ...
            updateTitle(false);
            MediaController.getInstance().checkIsFavorite(getApplicationContext(), mSongDetail, img_Favorite);
        }
    }
    ...
    public void newSongLoaded(Object... args) {
        MediaController.getInstance().checkIsFavorite(getApplicationContext(), (SongDetail) args[0], img_Favorite);
    }
